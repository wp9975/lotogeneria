...
import MiniLottoInteractiveNumberBoard from "./MiniLottoInteractiveNumberBoard ";
...
const MiniLottoOptions = ({ onGenerateNumbers }) => {
  const [numSets, setNumSets] = useState(1);
  const [uniqueNumbers, setUniqueNumbers] = useState(false);
  const [numberRange, setNumberRange] = useState([1, 42]);
  ...
  const generateNumbers = () => {
    ...
    for (let i = 0; i < numSets; i++) {
      ...
      if (requiredNumbers.length === 5) {
        numbers = new Set(requiredNumbers);
      } else {
        while (numbers.size < 5 - requiredNumbers.length) {
          const randomNumber =
  ...
  return (
    <Fragment>
      ...
      <div>
        <MiniLottoInteractiveNumberBoard
...
export default MiniLottoOptions;
